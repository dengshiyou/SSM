...
function initColTables()
{
	$('#queryTableList').datagrid({
       ...
       fitColumns: true,
       loadMsg: 'loading',
       url:"<%=path%>/taskinfo/bvmbillinfo/BvmBillInfo/getPageInfo?billType=0&flag=fj",
       ...
       },
       {
		   ...
		   formatter:function(value,rowData,rowIndex)
		   {
			   ...
			   {
				     str += "<input type=\"button\" class=\"ckmx_btn\" style=\"width:65px;\" title=\"查看配箱\" value=\"查看配箱\" onclick=\"doViewPx("+rowIndex+");\" />&nbsp;";
				}
			   else
			   {
				    if(startTime<nowTime){
					     ...
					     str += "<input type=\"button\" class=\"delete_btn\" title=\"删除\" value=\"删除\"  onclick=\"doDelete("+rowIndex+");\" />&nbsp;";
						 str += "<input type=\"button\" class=\"px_btn\" style=\"width:65px;\" title=\"附加配箱\" value=\"附加配箱\" onclick=\"doPx("+rowIndex+");\" />&nbsp;";
...
function doAdd()
{
	var URL="<%=path%>/taskinfo/bvmbillinfo/BvmBillInfo/loadAddBO?bo.billType=0";
...
function doEdit(rowIndex)
{
	var row = $("#queryTableList").datagrid("getRows");
    var dataRow = row[rowIndex];
	var URL="<%=path%>/taskinfo/bvmbillinfo/BvmBillInfo/loadUpdateBO?bo.billType=0&bo.oid="+dataRow.oid;
...
function doDelete(rowIndex)
{
    ...
    var dataRow = row[rowIndex];
	obj.messageTips = "任务";
	obj.messageTips2 = "删除失败，任务还有配箱数据或配置了已选机构!";
...
function doAddNodes(rowIndex){
	var row = $("#queryTableList").datagrid("getRows");
    var dataRow = row[rowIndex];
	var URL="<%=path%>/baseinfo/comutil/ComUtil/getPageInfo?bo.billId="+dataRow.oid+"&bo.billType=wdTabs&bo.startQueryTime="+dataRow.taskStartTime;
	...
	window.parent.parent.openDialog(URL,"选择机构",dWidth,dHeight); 		
}

function doPx(rowIndex){
	var row = $("#queryTableList").datagrid("getRows");
	var dataRow = row[rowIndex];
	var URL="<%=path%>/taskinfo/bvmtasknodedetailinfo/BvmTasknodeDetailinfo/loadBillNodes?bo.taskOid="+dataRow.oid;
